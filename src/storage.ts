...
export function store_wallet(wallet_name: string, encrypted_priv_key: Uint8Array, salt: Uint8Array, iv: Uint8Array) {
    localStorage.setItem(wallet_name, JSON.stringify({
        iv: [].map.call(iv, (x => x)),
...
export function get_wallet(wallet_name: string): WalletCryptoData {
    let data  = JSON.parse(localStorage.getItem(wallet_name));
    // if (data == null)
