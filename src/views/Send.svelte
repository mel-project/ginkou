<script lang="ts">
    ...
    if ($current_wallet == null) {
      ...
      if ($current_wallet_dump && $current_wallet_dump.summary.locked) {
        ...
        await prepare_mel_tx(
          $current_wallet,
          to_addrs.map((contact) => contact.address.trim()),
          [new BigNumber(send_amount)]
        )
          .ifLeft((err) => dsptch_err(err))
          .ifRight((tx) => {
  ...
  const search_contacts = (
    ...
    const predictions = [...contacts, ...wallet_contacts].filter(
      (contact) =>
        contact.name.indexOf(sub_name) >= 0 ||
        contact.address.indexOf(sub_name) >= 0
    );
