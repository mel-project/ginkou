...
export function get_wallet(wallet_name: string): WalletCryptoData {
  let data = JSONbig.parse(
    (localStorage.getItem(wallet_name) as unknown) as string
  );
  // if (data == null)
...
function assert_object_fields<T extends Object>(default_object: T) {
  return (test_object: T): Maybe<T> => {
    ...
    const property_test = (acc: Boolean, obj: [string, Object]) =>
      acc && test_object.hasOwnProperty(obj[0]);
    if (Object.entries(default_object).reduce(property_test, true))
      return Just(test_object);
    return Nothing;
  };
}

// for every entry in an object, map a function onto it's value and return the reconstructed object
function object_map<T, F>(
  obj_entries: [string, T][],
  func: (name: string, obj: T) => F
) {
  console.log(obj_entries);
  return Object.assign(
    {},
    ...obj_entries.map((entry: [string, T]) => ({
      [entry[0]]: func(entry[0], entry[1]),
    }))
  );
}
...
const get_saved_setting = (localName: string): Maybe<PersistentValue> => {
  ...
  let maybe_value: Maybe<PersistentValue> = saved_value
    ? Just(Object.assign({}, JSONbig.parse(saved_value)))
    : Nothing;
  // console.log(maybe_value.extract(), "maybed")
  return maybe_value.chain(assert_object_fields({ name: "", value: "" }));
};

const get_all_saved_settings = (
  storage_name: string,
  default_settings: PersistentStorage
): PersistentStorage => {
  return (default_settings
    .map((obj: PersistentValue) => {
      return get_saved_setting(storage_name + obj.name);
    })
    .filter((m: Maybe<PersistentValue>) => m.isJust())
    .map((m: Maybe<PersistentValue>) =>
      m.extract()
    ) as unknown) as PersistentStorage;
};

const config_to_persistent = (
  name: string,
...
//   })
// }
const restore_all_settings = (
  storage_name: string,
  setting_types: State<SettingConfig>
): State<Writable<any>> => {
  ...
  const assert_state = assert_object_fields(default_state_obj())(
    (setting_types as unknown) as State<string>
  );
  if (!assert_state)
    console.warn(
      "store.ts::restore_all_settings\nsettings_types doesn't satisfy type State<T>, be sure all fields in state are satisfied"
    );
  // this is what the storage state will look like on first start
  const default_storage: PersistentStorage = Object.entries(
    setting_types
  ).map((entry: [string, SettingConfig]) =>
    config_to_persistent(entry[0], entry[1])
  );

  // the previous saved settings state
  const saved_storage: PersistentValue[] = get_all_saved_settings(
    storage_name,
    default_storage
  );

  const default_state: State<PersistentValue> = Object.assign(
    {},
    ...default_storage.map((pv: PersistentValue) => ({ [pv.name]: pv }))
  );
  const saved_state: State<PersistentValue> = Object.assign(
    {},
    ...saved_storage.map((pv: PersistentValue) => ({ [pv.name]: pv }))
  );
  const reconstructed_state: State<PersistentValue> = Object.assign(
    default_state,
    saved_state
  );

  const state: State<Writable<any>> = Object.assign(
    {},
    ...Object.entries(reconstructed_state).map(
      (entry: [string, PersistentValue]) => ({
        [entry[0]]: persistent_to_writable(storage_name, entry[1]),
      })
    )
  );
    ...
    ($name, set: (a0: any) => void) => {
      if (!$name) {
        ...
        const refresh = async () => {
          ...
          console.log("attempting a dump of " + $name);
          console.log($name);
          const dump = await wallet_dump($name as string);
          dump
            .ifLeft((e) =>
              console.error(`error encountered in wallet_dump: ${e}`)
            )
            .map((dump) => {
